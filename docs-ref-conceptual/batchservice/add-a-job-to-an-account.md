...
## Remarks
 ...
 If auto pool creation fails, the Batch service moves the job to a Completed state, and the pool creation error is set in the job’s schedulingError property. For example, if any of the certificates specified in the certificateReferences for the auto pool does not exist, or is in the deleting or deletefailed state, the auto pool creation will fail, and the Batch service will terminate the job.
