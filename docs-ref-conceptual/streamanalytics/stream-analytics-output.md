...
|**rowKey**|Yes|This element indicates the name of a column from the **SELECT** statement in the query. Ensure that the value of row key column does not contain [invalid characters](https://docs.microsoft.com/en-us/rest/api/storageservices/understanding-the-table-service-data-model#characters-disallowed-in-key-fields).|  
