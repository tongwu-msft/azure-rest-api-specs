...
|`fuzzy=[boolean] (optional, default = false)`|When set to true, this API finds suggestions even if there is a substituted or missing character in the search text <sup>(1)</sup>. This provides a better experience in some scenarios but it comes at a performance cost as fuzzy suggestion searches are slower and consume more resources.|
...
(1) Using the fuzzy parameter, autocomplete suggestions can be found for misspelt partial input tokens but it doesn't guarantee inclusion of all candidate fuzzy matches. The fuzziness is limited to an edit distance of 1 unlike [fuzzy parameter in search](https://docs.microsoft.com/azure/search/search-query-fuzzy), reducing the size of the potential fuzzy matches. Moreover, the fuzzy step happens after the partial token expansion and only terms from the same index partition are considered for fuzzy matches. This increases performance of Autocomplete API by eliminating the aggregation step. The trade-off works well on real-world indexes where potential fuzzy matches get distributed evenly across each partition. However, the results will not include a candidate fuzzy term from a different partition, which usually happens when there are  few terms in the index.
