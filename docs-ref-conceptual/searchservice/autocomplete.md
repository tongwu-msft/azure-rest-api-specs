...
|---------------|-----------------|  
|`search=[string]`|The search text to complete. Must be at least 1 character, and no more than 100 characters. It cannot contain operators, query syntax, or phrases. |  
...
|`fuzzy=[boolean] (optional, default = false)`|Incorporates simple spell-check in the results. When set to true, this API finds suggestions even if there is a substituted or missing character in the search text. This provides a better experience in some scenarios, but it comes at a performance cost as fuzzy suggestion searches are slower and consume more resources. Although similarly named, this API is not the same as fuzzy search as exposed in the full Lucene query syntax.|  
