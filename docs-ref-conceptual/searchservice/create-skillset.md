...
|cognitiveServices | An all-in-one key is required for billable skills that call Cognitive Services APIs  on more than 20 documents daily, per indexer.  The key must be for a resource in the same region as the search service. For more information, see [Attach a Cognitive Services resource](/azure/search/cognitive-search-attach-cognitive-services). </br></br>You can omit this section if your skillset consists of only custom skills and utility skills (Conditional, Shaper, Text Merge, Text Split). The [Custom Entity Lookup](/azure/search/cognitive-search-skill-custom-entity-lookup) skill is a special case. It's not billable, nor is it associated with Cognitive Services, but an all-in-one key is required to relax the 20-document limit. |
