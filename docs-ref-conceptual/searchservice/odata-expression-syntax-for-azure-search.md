...
> [!NOTE]  	
>  Use `search.in(Id, '123, 456, ...')` instead of equality expression (`Id eq 123 or Id eq 456 or ...`) to expect sub-second response time for hundreds to thousands of values. Note that there is no explicit limit on the number of items you can pass to `search.in`, up to the maximum request size, however the latency will grow as the number of values grows.
