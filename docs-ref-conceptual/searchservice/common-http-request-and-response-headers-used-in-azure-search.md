...
|authorization (preview) |String|The OAuth 2.0 access token for the request. Requires a [preview sign up](https://aka.ms/azure-cognitive-search/rbac-preview), followed by search service configuration for data plane authentication. You can use the portal or [Management REST API 04-01-2021-Preview](/rest/api/searchmanagement/2021-04-01-preview/services/create-or-update) (or an equivalent SDK) to configure the search service. You must also have [Azure role assignments](/azure/search/search-security-rbac) scoped to the search service or top-level resources, and your client code must provide the token. This request header can be used in any supported REST API version as long as the search service is configured for [data plane authentication](/rest/api/searchmanagement/2021-04-01-preview/services/create-or-update#dataplaneaadorapikeyauthoption).  |  
