...
> [!NOTE]  
>  Even though the POST request size limit is very large, filter expressions cannot be arbitrarily complex. For more information about filter complexity limitations, see [OData Expression Syntax for Azure Search](https://docs.microsoft.com/azure/search/query-odata-filter-orderby-syntax).  
...
|---------------|-----------------|  
|`search=[string]`|The search text to use to suggest queries. Must be at least 1 character, and no more than 100 characters. It cannot contain operators, query syntax, or phrases. |  
...
|`fuzzy=[boolean] (optional, default = false)`|Incorporates simple spell-check in the results. When set to true, this API finds suggestions even if there is a substituted or missing character in the search text. While this provides a better experience in some scenarios, it comes at a performance cost, as fuzzy suggestion searches are slower and consume more resources. Although similarly named, this API is not the same as fuzzy search as exposed in the full Lucene query syntax.|  
