...
|**analyzer**|The name of the lexical analyzer to use for the field. Lexical analysis removes stop words, reduces words to root forms, and more. The default is `Standard Lucene`. For a list of allowed values, see [Analyzers](https://docs.microsoft.com/azure/search/search-analyzers). This attribute can be used only with **searchable** simple fields and not [complex fields](https://docs.microsoft.com/azure/search/search-howto-complex-data-types) (must be `null`), and it can't be set together with either **searchAnalyzer** or **indexAnalyzer**. Once the analyzer is chosen, it cannot be changed for the field. For more information about analysis during query and indexing operations, see the ["Lexical Analysis" section in How full text search works](https://docs.microsoft.com/azure/search/search-lucene-query-architecture#stage-2-lexical-analysis).|  
