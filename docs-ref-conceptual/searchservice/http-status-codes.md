...
|499 Client Closed Request| This error is never returned in a response, but you might see it in log files. It indicates that a client closed the request before a response was returned by the service. When Azure Search detects that a request was canceled (for example, through .Net’s cancellation token mechanism) and the cancellation was not requested by service-side code, a 499 error is logged. |
