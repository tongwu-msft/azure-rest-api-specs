...
> [!Important]
> This preview adds support for properties used by indexers connecting to external data sources using a user-managed identity. Properties include **SearchIndexerDataIdentity**, **SearchIndexerDataNoneIdentity**, and **SearchIndexerDataUserAssignedIdentity**. **SearchIndexerDataIdentity** is both a top-level property and also a child property of the encryptionKey. 

In Azure Cognitive Search, a data source is used with [indexers](../create-indexer.md), providing the connection information for ad hoc or scheduled data refresh of a target index, pulling data from [supported data sources](/azure/search/search-indexer-overview#supported-data-sources). 
...
|identity| Optional. It contains a `userAssignedIdentity` of type `#Microsoft.Azure.Search.DataUserAssignedIdentity` and specifies a user-managed identity under which the search service runs, and for which a role assignment exists that grants read permissions to the resource. |
...
**Example: Connect using Azure role-based access and a user-managed identity (preview)**
