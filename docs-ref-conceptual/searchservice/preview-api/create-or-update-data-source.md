...
|credentials|Required. It contains a `connectionString` property that specifies the connection string for the data source. If you are using a [managed identity to authenticate](/azure/search/search-howto-managed-identities-data-sources), you can omit explicit credentials on the connection string. 
