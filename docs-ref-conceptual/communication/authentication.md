...
Once you have a suitable User Access Token, you can include it in your requests to Azure Communication Services' REST API by supplying it within the `Authorization` header using the Bearer HTTP authentication scheme `Authorization: Bearer <token>`.
