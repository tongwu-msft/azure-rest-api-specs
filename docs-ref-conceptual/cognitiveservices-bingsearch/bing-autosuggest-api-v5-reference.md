...
description: Describes the Autosuggest API's function and provides technical details on query parameters and the JSON responses to expect from suggested queries.
...
|<a name="subscriptionkey" />Ocp-Apim-Subscription-Key|Required request header.<br /><br /> The subscription key that you received when you signed up for this service in [Cognitive Services](https://azure.microsoft.com/services/cognitive-services/).|  
