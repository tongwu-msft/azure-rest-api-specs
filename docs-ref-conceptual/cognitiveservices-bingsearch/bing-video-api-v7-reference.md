...
|<a name="query" />q|The user's search query string. The query string cannot be empty.<br /><br /> The query string may contain [Bing Advanced Operators](http://help.bing.microsoft.com/#apex/18/en-US/10001/-1). For example, to limit videos to a specific domain, use the **site:** operator.<br /><br /> Use this parameter only with the Video Search API. Do not specify this parameter when calling the Trending Videos API.|String|Yes|  
