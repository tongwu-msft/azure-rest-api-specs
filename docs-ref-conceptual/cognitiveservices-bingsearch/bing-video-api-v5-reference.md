...
|<a name="freshness" />freshness|Filter videos by age. Age refers to the date and time that the video was discovered by Bing. The following are the possible filter values.<br /><ul><li>Day&mdash;Return videos discovered within the last 24 hours<br /></li><li>Week&mdash;Return videos discovered within the last 7 days<br /><br /></li><li>Month&mdash;Return videos discovered within the last 30 days</li><li>Year&mdash;Return images discovered within the last year</li></ul>|String|No|  
...
|<a name="offset" />offset|The zero-based offset that indicates the number of videos to skip before returning videos. The default is 0. The offset should be less than (totalEstimatedMatches - `count`).<br /><br /> Use this parameter along with the `count` parameter to page results. For example, if your user interface presents 20 videos per page, you would set  `count` to 20 and `offset` to 0 to get the first page of results. For each subsequent page, you would increment `offset` by 20 (for example, 0, 20, 40).<br /><br /> It is possible for multiple pages to include some overlap in results. To prevent duplicates, see nextOffsetAddCount.<br /><br /> This parameter is supported only by the Video API; do not specify this parameter when calling the Trending Videos API or the Search API.|Unsigned Short|No|  
...
|<a name="video-duration" />duration|The video's duration or length. For example, PT2M50S. For information about the format, see [Durations](https://en.wikipedia.org/wiki/ISO_8601#Durations) on Wikipedia.com.|String|  
