...
|----------|-----------|----------|--------------|
|<a name="answercount" />answerCount|The number of answers that you want the response to include. The answers are chosen based on ranking. For example, Bing returns webpages, images, videos, and relatedSearches for a request. If you set this parameter to two (2), the response will include webpages and images.<br /><br />If you included the `responseFilter` query parameter in the same request and set it to webpages and news, the response would include only webpages.<br /><br />Also, see [promote](#promote).|Unsigned Integer|No
...
|<a name="promote" />promote|A list of answers that you want the response to include regardless of their ranking. For example, to promote videos and images into the response, you’d set `promote` to *videos,images*. This differs from `responseFilter` which includes answers based on ranking. For example, if you set `responseFilter` to *webpages,images,news*, Bing may return only webpages and images based on ranking. If it's important that the response includes images and news, set `promote` to *images,news*. Bing will return images and news, if they exist, no matter their ranking. <br /><br /> The following are the possible values.<br /><ul><li>Computation</li><li>Images</li><li>News</li><li>RelatedSearches</li><li>SpellSuggestions</li><li>TimeZone</li><li>Videos</li><li>Webpages</li></ul><br />The answers to promote do not count against the [answerCount](#answercount) limit. For example, if `answerCount` is 2 and promote is set to *videos,images*, the response could include webpages, news, videos, and images.|String|No 
