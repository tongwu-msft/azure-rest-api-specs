...
The Autosuggest API lets partners send a partial search query term to Bing and get back a list of suggested queries that other users have searched on. In addition to including searches that other users have made, the list may include suggestions based on user intent.  

Typically, you use this API to support a richer search box experience where you display a drop-down list of suggested query strings as the user types a query into the search box.  
...
The following are the query parameters that the request may include. The Required column indicates whether you must specify the parameter. The query parameter values must be URL encoded.  
...
|[QueryContext](#querycontext)|Defines the query string that Bing used for the request.|  
...
|Name|Value|Type|  
|----------|-----------|----------|  
|displayText|The suggested query string to display in a user interface. Typically, you add the suggested query string to a drop-down list of suggested queries.|String|  
|<a name="searchaction-query" />query|The suggested query string.<br /><br /> If the user selects the query string from the drop-down list, use the query string in a Bing API request and display the search results yourself. Or, you could use the URL in the `url` field to send the user to the Bing search results page for the suggested query.|String|  
|<a name="searchaction-searchkind" />searchKind|The type of suggestion. The following are the possible values.<br /><br />&nbsp;&nbsp;* CustomSearch&mdash;The suggestion is from a non-Web search suggestion data source.<br />&nbsp;&nbsp;* WebSearch&mdash;The suggestion is from a web search suggestion data source.|String|  
|<a name="searchaction-url" />url|The URL that takes the user to the Bing search results page for the suggested query.|String|  
|urlPingSuffix|A query string fragment that identifies the `url` link. For details about using this fragment, see the [pingUrlBase](#pingurlbase) field.|String|  
...
|Name|Value|Type|  
|----------|-----------|----------|  
|<a name="suggestgroup-name" />name|The name of the group. The name identifies the type of suggestions that the group contains. For example, web search suggestions. The following are the possible group names.<br /><br />&nbsp;&nbsp;* Custom&mdash;The group contains suggestions from a non-web search suggestions data source.<br />&nbsp;&nbsp;* Web&mdash;The group contains web search suggestions.|String|  
...
|----------|-----------|----------|  
|_type|Type hint.|String|  
|instrumentation|The Bing instrumentation URLs.|[Instrumentation](#instrumentation)|  
