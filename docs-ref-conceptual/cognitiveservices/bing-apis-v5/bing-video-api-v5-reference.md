...
|<a name="freshness" />freshness|Filter videos by age. Age refers to the date and time that the video was discovered by Bing. The following are the possible filter values.<br /><br />&nbsp;&nbsp;* Day&mdash;Return videos discovered within the last 24 hours<br />&nbsp;&nbsp;* Week&mdash;Return videos discovered within the last 7 days<br />&nbsp;&nbsp;* Month&mdash;Return videos discovered within the last 30 days|String|No|  
...
|<a name="modulesrequested" />modulesRequested|A comma-delimited list of one or more insights to request. (When you URL encode the query string, the commas change to %2C.) The following are the possible case-insensitive values.<br /><br />&nbsp;&nbsp;* All&mdash;Return all available insights.<br /><br/>&nbsp;&nbsp;* RelatedVideos&mdash;Return a list of videos that are similar to the video specified by the `id` query parameter.<br /><br/>&nbsp;&nbsp;* VideoResult&mdash;Return the video that you're requesting insights of (this video is the video that you set the `id` query parameter to in your insights request).<br /><br /> If you specify an insight and there is no data for it, the response object will not include the related field. For example, if you specify RelatedVideos and none exist, the response will not include the `relatedVideos` field.<br /><br /> Although the user's query string is not required, you should always include it because it helps to improve relevance and the results.<br /><br /> This parameter is supported only by the `/videos/details` endpoint; do not specify this parameter when calling the `/videos` endpoint or the Search API.|String|Yes|  
...
|Name|Value|Type|  
|----------|-----------|----------|  
|<a name="pricing" />pricing|Filter videos by price. The following are the possible filter values.<br /><br />&nbsp;&nbsp;* Free&mdash;Return videos that are free to view<br />&nbsp;&nbsp;* Paid&mdash;Return videos that require a subscription or payment to view<br />&nbsp;&nbsp;* All&mdash;Do not filter by pricing. Specifying this value is the same as not specifying the `pricing` parameter.||  
|<a name="resolution" />resolution|Filter videos by resolution. The following are the possible filter values.<br /><br />&nbsp;&nbsp;* 480p&mdash;Return videos with a 480p or higher resolution<br />&nbsp;&nbsp;* 720p&mdash;Return videos with a 720p or higher resolution<br />&nbsp;&nbsp;* 1080p&mdash;Return videos with a 1080p or higher resolution<br />&nbsp;&nbsp;* All&mdash;Do not filter by resolution. Specifying this value is the same as not specifying the `resolution` parameter.||  
|<a name="videolength" />videoLength|Filter videos by length. The following are the possible filter values.<br /><br />&nbsp;&nbsp;* Short&mdash;Return videos that are less than 5 minutes<br />&nbsp;&nbsp;* Medium&mdash;Return videos that are between 5 and 20 minutes, inclusive<br />&nbsp;&nbsp;* Long&mdash;Return videos that are longer than 20 minutes<br />&nbsp;&nbsp;* All&mdash;Do not filter by length. Specifying this value is the same as not specifying the `videoLength` parameter.|String|  
...
|<a name="query-thumbnail" />thumbnail|AThe URL to a thumbnail of a related image.<br /><br /> The object includes this field only for pivot suggestions and related searches.|[Thumbnail](#thumbnail)|  
...
<a name="video"></a>   
### Video  
An object that contains information about a video that is relevant to the query.  
...
|<a name="videos-scenario" />scenario|The scenario that reflects the query's intent. The following are the possible values.<br /><br />&nbsp;&nbsp;* List&mdash;For scenarios where there's more than one video that matches the user's intent.<br />&nbsp;&nbsp;* SingleDominantVideo&mdash;For scenarios where there's a single music video that matches the user's request (the Videos answer contains only one music video). This scenario is set only for music videos.<br /><br /> The object includes this field only when the answer is part of the Web Search API response; the Video Search API response will not include this field.|String|  
|<a name="videos-totalestimatedmatches" />totalEstimatedMatches|The estimated number of videos that match the query. Use this number along with the [count](#count) and [offset](#offset) query parameters to page the results.<br /><br /> The response includes this field only when calling the Video Search API; the Web Search API will not include this field in the response.|Long|  
