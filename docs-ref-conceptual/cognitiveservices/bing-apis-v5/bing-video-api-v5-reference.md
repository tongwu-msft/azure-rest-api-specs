...
|<a name="freshness" />freshness|Filter videos by age. Age refers to the date and time that the video was discovered by Bing. The following are the possible filter values.<br /><br /><ul><li>Day&mdash;Return videos discovered within the last 24 hours<br /></li><li>Week&mdash;Return videos discovered within the last 7 days<br /></li><li>Month&mdash;Return videos discovered within the last 30 days</li></ul>|String|No|  
...
|<a name="videos-scenario" />scenario|The scenario that reflects the query's intent. The following are the possible values.<br /><br /></ul><li>List&mdash;For scenarios where there's more than one video that matches the user's intent.<br /></li><li>SingleDominantVideo&mdash;For scenarios where there's a single music video that matches the user's request (the Videos answer contains only one music video). This scenario is set only for music videos.</li></ul><br /><br /> The response includes this field only when calling Web Search API. The Video Search API response does not include this field.|String|  
