...
The client/resource interactions for this grant are very similar to step #2 of the authorization code grant. Please see the "Get a token" section in [Microsoft identity platform and the OAuth 2.0 client credentials flow][AAD-Oauth-Client-Creds] for details on the format of the HTTPS POST request to the `/token` endpoint, and example request/response messages.
