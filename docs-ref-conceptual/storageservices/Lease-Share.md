...
The `Lease Share` operation establishes and manages a lock on a share for write operations. The lock duration can be 15 to 60 seconds, or can be infinite. The `Lease Share` operation can be called in one of five modes:
...
|`x-ms-lease-id: <ID>`|Required to renew, change, or release the lease.<br /><br /> The value of `x-ms-lease-id` can be specified in any valid GUID string format. See [Guid Constructor (String)](https://docs.microsoft.com/dotnet/api/system.guid.-ctor#System_Guid__ctor_System_String_) for a list of valid GUID string formats.|  
...
|`x-ms-version`|Indicates the version of the File REST API used to execute the request.|  
