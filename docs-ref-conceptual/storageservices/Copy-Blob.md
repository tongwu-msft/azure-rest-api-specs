...
|`x-ms-sync-copy`|Optional. In version 2017-11-09 and newer, setting this header to `true` completes the `Copy Blob` operation synchronously if the source is an block blob. This header is defaulted to `false`, and the `Copy Blob` operation will complete asynchronously if this header is not specified. |
...
## Remarks  
 ...
 In version 2017-11-09 and newer, the `Copy Blob` operation can complete synchronously by using the `x-ms-sync-copy` header (see Request Header section above for more details). If the synchronous `Copy Blob` operation is used, the source blob must be a block blob. The copy ID returns from the operation in this case is invalid. Using this copy ID for other operation gives undefined behaviors.
