...
|`x-ms-requires-sync`|Optional. In version 2018-03-28 and newer, setting this header to `true` completes the `Copy Blob` operation synchronously if the source is an block blob. This header defaults to `false`, and the `Copy Blob` operation may complete asynchronously if this header is not specified. |
|`x-ms-source-content-md5`|Optional. In version 2018-03-28 and later and when `x-ms-requires-sync` is `true`, this header may be provided with an MD5 hash of the copy source's contents. This hash is used to verify the transactional integrity of the copy source contents during transport.  When this header is specified, the storage service checks the MD5 hash of the copy source contents with the one that was sent.  If the two hashes do not match, the operation will fail with error code 400 (Bad Request). |
...
|`Content-MD5`|In version 2018-03-28 and later and when `x-ms-requires-sync` is `true`, the MD5 hash of the source blob contents is computed by the server and sent as part of the response.|
...
## Remarks  
 ...
 In version 2018-03-28 and newer, the `Copy Blob` operation can complete synchronously by using the `x-ms-requires-sync` header (see Request Header section above for more details). If the synchronous `Copy Blob` operation is used, the source blob must be a block blob. The copy ID returned from the operation in this case is invalid. Using this copy ID for other operation gives undefined behaviors.
