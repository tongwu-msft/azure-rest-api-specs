...
| `x-ms-file-permission-key` | Required. Key of the permission to be set for the directory.<br /><br />Note that only one of `x-ms-file-permission` or `x-ms-file-permission-key` can be specified. |
...
| `x-ms-file-last-write-time` | Required. The last modified property for a directory. The date/time format follows ISO 8601 format. Example 2017-05-10T17:52:33.9551861Z. When not specified, the default value is now (i.e. the time of file creation). |
