...
For version 2020-06-12 and above, for Hierarchical Namespace enabled accounts, `List Blobs` returns `Owner`, `Group`, `Permissions` and `Acl` element when the request contains `include={permissions}` parameter. Please note that `Acl` element will be a combined list of Access and Default Acl's that were set on the file/directory.
