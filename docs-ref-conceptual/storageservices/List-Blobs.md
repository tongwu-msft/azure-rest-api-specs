...
|`showonly={deleted}`|Optional. Version 2020-08-04 and newer. Only for accounts with Hierarchical Namespace enabled accounts. When a request includes this parameters, the list only contains soft deleted blobs. If include=deleted is also specified, the request will fail with Bad Request (400).| 
