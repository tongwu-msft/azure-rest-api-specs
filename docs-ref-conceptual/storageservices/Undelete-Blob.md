...
|`x-ms-undelete-source`|Optional. Version 2020-08-04 and newer. Only for accounts with Hierarchical Namespace enabled accounts. The path of the soft deleted blob to undelete. The format is `blobPath?deletionid=<id>` or `blobPath?snapshot=<DateTime>&deletionid=<id>`. The account and container name are not included in the path. `DeletionId` is the unique identifier of the soft deleted blob. It can be retrieved by listing soft deleted blobs with the List Blobs REST API for Hierarchical Namespace enabled accounts. The snapshot parameter is an opaque datetime value that, when present, specifies the soft deleted snapshot to undelete. The path should be percent encoded.| 
