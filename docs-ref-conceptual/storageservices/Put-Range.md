...
| `x-ms-write: { update ¦ clear }` | Required. You must specify one of the following options: <ul><li>`update`: Writes the bytes specified by the request body into the specified range. The `Range` and `Content-Length` headers must match to perform the update.</li><li>`clear`: Clears the specified range and releases the space used in storage for that range. To clear a range, set the `Content-Length` header to zero, and set the `Range` header to a value that indicates the range to clear, up to maximum file size.</li></ul> |
