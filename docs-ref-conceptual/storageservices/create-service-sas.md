...
The following table describes whether to use the `signedIp` based on the client environment and the location of the storage account.
...
| Client running on-premises or in a different cloud environment | In any Azure region | A SAS provided to the client in this scenario may include a public IP address or range of addresses for the `signedIp` field. A request made with the SAS must originate from the specified IP address or range of addresses.<br> <br> <br> <br>If the request passes through a proxy or gateway, then use the public outbound IP address of that proxy or gateway. |
  
> [!IMPORTANT]
> A SAS used by a client that is in the same Azure region as the storage account should not include a public outbound IP address for the `signedIp` field. Requests made from within the same region using a SAS with a public outbound IP address specified will fail.
