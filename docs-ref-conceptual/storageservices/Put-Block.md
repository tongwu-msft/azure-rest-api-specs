...
|`Content-Length`|Required. The length of the block content in bytes. The block must be less than or equal to 4 GB (4000 MB) in size for version 2012-12-12 and later (Preview). See the Remarks for limits in older versions.<br /><br />When the length is not provided, the operation will fail with the status code 411 (Length Required).|  
...
## Remarks  
 `Put Block` uploads a block for future inclusion in a block blob. A block blob can include a maximum of 50,000 blocks. Each block can be a different size, up to a maximum of 4 GB (4000 MB) for version 2020-12-12 and later (Preview), 100 MB for version 2016-05-31 and later, and 4 MB for older versions. The maximum size of a block blob is therefore approximately 200 TB (4 GB X 50,000 blocks) for version 2020-12-12 and later (Preview), 4.75 TB (100 MB X 50,000 blocks) for version 2016-05-31 and later, and 195 GB (4 MB X 50,000 blocks) for all older versions. 

 A blob can have a maximum of 100,000 uncommitted blocks at any given time. The set of uncommitted blocks cannot exceed 400 TB in total size. If these maximums are exceeded, the service returns status code 409 (RequestEntityTooLargeBlockCountExceedsLimit).
