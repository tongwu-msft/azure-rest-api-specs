...
|`x-ms-copy-source-tag-option`|Optional. Possible values are REPLACE or COPY (case-sensitive). Copies the tags from source blob if COPY. Source blob must be non-public, and request must have permission to read tags on the source blob and write tags on the destination blob. This incurs an extra rest getblobtags call on the source account. REPLACE will set tags specified by x-ms-tags if that header is present. No tags otherwise. If this header is not specified, REPLACE is assumed as the default behavior. Conflicts with x-ms-tags option if COPY is specified. Supported in version 2021-04-10 and newer. |
