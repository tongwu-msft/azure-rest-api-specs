...
|`x-ms-or-policy-id_rule-id`|Version 2019-10-10 or newer. For block blob only, valid values are `complete`/`failed`. `policy-id` is a guid which means the id of object replication policy on account level, `rule-id` is a guid which means the id of rule on container level within a policy. If the account is `ObjectReplication` enabled and the blob is in source container, this status means the replication status of blob with the policy id and rule id. `complete` means this blob is replicated succesfully to dest container, failed means this blob is replicated failed to dest container. |  
|`x-ms-or-policy-id`|Version 2019-10-10 or newer. For block blob only, valid value is a guid. If the account is `ObjectReplication` enabled and the blob is in destination container, this value means this blob is replicated by this object replication policy. |  
