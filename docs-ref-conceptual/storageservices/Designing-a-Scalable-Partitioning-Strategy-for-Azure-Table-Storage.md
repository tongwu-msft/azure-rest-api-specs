...
|Unique values|Many small partitions|The table is highly scalable.<br /><br /> Range partitions might improve the performance of cross-partition range queries.|Queries that involve ranges might require visits to more than one server.<br /><br /> Batch transactions are not possible.<br /><br /> Append-only or prepend-only patterns might affect insert throughput.|  
...
##  <a name="gdft"></a> Choose the PartitionKey value
...
###  <a name="ttt"></a> Entity group transactions
...
###  <a name="trtrruyu"></a> Storage operations
...
Load balancing at the partition layer occurs when a partition gets too hot. When a partition is too hot, the partition, specifically the partition server, operates beyond its target scalability. For Azure storage, each partition has a scalability target of 500 entities per second. Load balancing also occurs at the Distributed File System (DFS) layer. 
