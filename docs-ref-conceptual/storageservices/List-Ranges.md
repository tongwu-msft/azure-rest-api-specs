...
|`x-ms-content-length`|The size of the file in bytes. When `prevsharesnapshot` is present then the value describes the size of the file at the `sharesnapshot` (if `sharesnapshot` query parameter is present) or live share. |  
...
Beginning with version 2020-02-10, you can call List Ranges with `prevsharesnapshot` parameter to return the ranges that differ between the live file and a snapshot, or between two snapshots of the file on snapshots created after TBD date. Using these range differences, you can retrieve an incremental snapshot of a file. Incremental snapshots are cost-effective way to backup files if you wish to implement your own backup solution. 
...
- [Generation IDs are different] 409 (Conflict) if called on a file that was the target of an overwriting Copy after the snapshot specified by `prevsharesnapshot` was taken.
- [File IDs are different] 409 (Conflict) if called on a file that was deleted and recreated with the same name and location after the snapshot specified by `prevsharesnapshot` was taken.
