...
1. Convert each HTTP header name to lowercase.
...
1. Finally, append a new-line character to each canonicalized header in the resulting list. Construct the `CanonicalizedHeaders` string by concatenating all headers in this list into a single string. 
...
1. Append the resource's encoded URI path, without any query parameters.
  
1. Retrieve all query parameters on the resource URI, including the `comp` parameter if it exists.
  
1. Convert all parameter names to lowercase.
  
1. Sort the query parameters lexicographically by parameter name, in ascending order. 
  
1. URL-decode each query parameter name and value. 
  
1. Include a new-line character (\n) before each name-value pair.

1. Append each query parameter name and value to the string in the following format, making sure to include the colon (:) between the name and the value:  
  
    `parameter-name:parameter-value`  
  
1. If a query parameter has more than one value, sort all values lexicographically, then include them in a comma-separated list:  
...
1. Append the resource's encoded URI path. If the request URI addresses a component of the resource, append the appropriate query string. The query string should include the question mark and the `comp` parameter (for example, `?comp=metadata`). No other parameters should be included on the query string. 
