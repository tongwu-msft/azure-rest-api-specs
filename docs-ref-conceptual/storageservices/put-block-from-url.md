...
|`Content-Length`|Required. Specifies the number of bytes being transmitted in the request body. The value of this header must be set to zero. When the length is not provided, the operation will fail with the status code 400 (Bad Request).|  
