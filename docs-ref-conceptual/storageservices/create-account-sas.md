...
|`SignedStart (st)`|Optional. The time at which the SAS becomes valid, expressed in UTC time and in an ISO 8601 format. If omitted, start time for this call is assumed to be the time when the storage service receives the request. For more information about supported UTC formats, see [Formatting DateTime values](formatting-datetime-values.md).|  
|`SignedExpiry (se)`|Required. The time at which the shared access signature becomes invalid, expressed in UTC time and in an ISO 8601 format. For more information about supported UTC formats, see [Formatting DateTime values](formatting-datetime-values.md).|  
