...
1. The **request URI**, which consists of: `{URI-scheme} :// {URI-host} / {resource-path} ? {query-string}`. Note that we are calling out the request URI separately here, because most languages or frameworks require you to pass it separately from the request message, but it's actually included in the request message header. 
...
For testing HTTP requests/responses, see:
