...
The Blob service API includes operations for enumerating the containers within an account (the [List Containers](../fileservices/List-Containers2.md) operation) and the blobs within a container (the [List Blobs](../fileservices/List-Blobs.md) operation). These operations have some common features worth noting.  
...
##  <a name="Subheading5"></a> XML Response Format  
 ...
 The `DateTime` value returned in the `Last-Modified` element is in RFC 1123 format. For more information about `DateTime` values, see [Representation of Date-Time Values in Headers](../fileservices/Representation-of-Date-Time-Values-in-Headers.md).  
